...
export function solveMeasureSpacing(
    ...
    const durationBeats = resolveMeasureDurationBeats(
        timeline,
        resolvedOptions.fallbackDurationBeats,
    );
    const pixelsPerBeat = durationBeats > 0 ? measure.contentWidth / durationBeats : 0;
    const visualWidthsByPath = new Map(
        measure.layout.contributions.map((contribution) => [contribution.path, contribution.width]),
    );
    ...
    const columns = Array.from(columnsByBeat.values()).sort(
