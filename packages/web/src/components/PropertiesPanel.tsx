...
export function PropertiesPanel() {
  ...
  const measureVoiceOptions = Array.from(
    { length: selectedMeasure?.voiceCount ?? 1 },
    (_, index) => ({
        ...
        <CardContent className="space-y-2.5">
          {selectedMeasure ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <Button variant="secondary" size="sm" onClick={addVoiceToSelection}>
              ...
              <div
                className={`flex items-center gap-1.5 border px-2 py-1.5 text-[11px] ${rhythmStatusClasses[selectedMeasure.rhythm.status]}`}
              >
                <Layers className="w-3.5 h-3.5" />
                <div className="min-w-0">
                  <div className="font-medium">
                    Voice {selectedMeasure.sequenceNumber} - {selectedMeasure.rhythm.label}
                  </div>
                  <div className="mt-0.5 font-mono text-[10px] opacity-90">
                    {getRhythmDetail(selectedMeasure.rhythm)}
                  </div>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div
                className={`flex items-center gap-1.5 border px-2 py-1.5 text-[11px] ${rhythmStatusClasses[selectedEvent.rhythm.status]}`}
              >
                <Layers className="w-3.5 h-3.5" />
                <div className="min-w-0">
                  <div className="font-medium">
                    Voice {selectedEvent.sequenceNumber} - {selectedEvent.rhythm.label}
                  </div>
                  <div className="mt-0.5 font-mono text-[10px] opacity-90">
                    {getRhythmDetail(selectedEvent.rhythm)}
                  </div>
                </div>
              </div>
                ...
                <>
                  <div className="grid grid-cols-[1fr_70px] gap-1.5">
                    <div className={compactFieldClass}>
                      ...
                      <Select
                        ...
                        onChange={(event) =>
                          updateSelectedNotePitch({ step: event.target.value as PitchStep })
                        }
                      />
                    </div>
                    <div className={compactFieldClass}>
                      ...
                      <Input
                        ...
                        onChange={(event) =>
                          updateSelectedNotePitch({ octave: Number(event.target.value) })
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className={compactFieldClass}>
                    ...
                    <Select
                      ...
                      value={
                        selectedAccidental === undefined ? 'none' : selectedAccidental.toString()
                      }
                  ...
                  <div className={compactFieldClass}>
                    ...
                    <Select
                      ...
                      onChange={(event) =>
                        updateSelectedNotehead(event.target.value as NoteheadOption)
                      }
                    />
                  </div>
              ...
              {parts.map((part) => (
                ...
                <div
                  ...
                  className={`
                                    ...
                                    `}
                >
                  <div className="w-1 h-6" style={{ background: part.color }} />
                  <div className="flex-1 text-left min-w-0">
                    {selectedPartId === part.id ? (
                      <div>
                        <Label htmlFor={`part-name-${part.id}`} className="sr-only">
                          Part Name
                        </Label>
                        <Input
                      ...
                      <>
                        <div className="truncate text-[12px] font-bold text-[#121212]">
                          {part.name}
                        </div>
                        {part.shortName && (
                          <div className="truncate text-[10px] text-[#5e625a]">
                            {part.shortName}
                          </div>
                        )}
                      </>
