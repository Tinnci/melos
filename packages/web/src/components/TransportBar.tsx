    ...
    <div className="flex items-center gap-3 border border-[#8f9289] bg-[#d4d6d0] px-2 py-1">
      ...
      <div className="flex gap-1">
        ...
        <Button
          ...
          className={
            status === 'playing'
              ? 'border-[#347200] bg-[#65bd13] hover:bg-[#76cf24] animate-pulse'
              : ''
          }
        >
      ...
      <div className="flex items-center gap-2 min-w-[170px]">
        <span className="text-[10px] font-black uppercase text-[#5e625a]">Tempo</span>
        <Slider
          min={40}
          max={200}
          value={tempo}
      ...
      <Badge
        variant={
          status === 'playing' ? 'success' : status === 'unsupported' ? 'destructive' : 'secondary'
        }
        className="gap-1"
      >
        {status === 'playing' && (
