...
export function ScoreCanvas() {
  ...
  const sourceMarkup = useMemo(() => (score ? JSON.stringify(score, null, 2) : ''), [score])

  useEffect(() => {
    document
      .querySelectorAll('.score-object.selected, .measure-hitbox.selected')
      .forEach((element) => {
    ...
    >
      <div className="relative z-10 flex items-center justify-between gap-2 border border-[#a6a8a0] bg-[#dedfd9] px-2 py-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="icon" title="Reset zoom" onClick={() => setZoom(1)}>
