...
export const noteheadOptions = ['default', 'x', 'diamond', 'triangle', 'slash', 'square', 'circle-x'] as const
...
function isEditableSelection(selection: Selection | null): selection is Selection & { type: EditableEventKind } {
    return selection?.type === 'note' || selection?.type === 'rest' || selection?.type === 'dynamic'
}

function isMeasureSelection(selection: Selection | null): selection is Selection & { type: 'measure' } {
...
function withSequenceAt(
    ...
    updater: (sequence: Score['parts'][number]['measures'][number]['sequences'][number]) => Score['parts'][number]['measures'][number]['sequences'][number]
): Score['parts'][number]['measures'][number] {
    const nextSequences = measure.sequences.length > 0
        ? [...measure.sequences]
        : [createEmptySequence()]
...
function insertContentEventAfter(
    ...
    for (const item of content) {
        ...
        if (Array.isArray(editable.content)) {
            const nested = insertContentEventAfter(editable.content as ScoreContentItem[], eventId, newEvent)
            if (nested.inserted) {
...
function resolveMeasureTarget(
    ...
    const partIndex = selectedPartId
        ? score.parts.findIndex((part) => part.id === selectedPartId)
        : 0
    if (partIndex < 0) return null
...
function removeVoiceFromMeasure(
    ...
    target: { partIndex: number; measureIndex: number },
    activeSequenceIndex: number
): { score: Score; sequenceIndex: number; removed: boolean; partId?: string; measureNumber: number } {
    const part = score.parts[target.partIndex]
    const measure = part.measures[target.measureIndex]
    const measureNumber = measure.index ?? target.measureIndex + 1
                    ...
                    ? {
                        ...
                        measures: scorePart.measures.map((partMeasure, measureIndex) => (
                            measureIndex === target.measureIndex
                                ? {
                                    ...partMeasure,
                                    sequences: partMeasure.sequences.filter((_, index) => index !== sequenceIndex),
                                }
                                : partMeasure
                        )),
                    }
...
function insertEvent(
    ...
    if (isEditableSelection(selection)) {
        ...
        if (inserted) return { score: nextScore, partId: insertedPartId, sequenceIndex: insertedSequenceIndex }
    }
    ...
    if (isMeasureSelection(selection)) {
        ...
        if (inserted) return { score: nextScore, partId: insertedPartId, sequenceIndex: targetSequenceIndex }
    }
    ...
    const nextScore: Score = {
        ...
        parts: score.parts.map((part, partIndex) => {
            ...
            const measures = part.measures.length > 0
                ? part.measures
                : [{ index: 1, sequences: [{ content: [] }] }]
            ...
            return {
                ...
                measures: measures.map((measure, measureIndex) => {
                    ...
                    const sequences = measure.sequences.length > 0
                        ? measure.sequences
                        : [{ content: [] }]
...
export const useScoreStore = create<ScoreState>()(
    subscribeWithSelector((set, get) => ({
        ...
        updateSelectedDynamic: (value) => {
            ...
            const result = updateEventById(score, selection.id, (item) => (
                item.type === 'dynamic'
                    ? { ...item, value } as EditableContentItem
                    : item
            ))
        ...
        getSelectedMeasure: () => getSelectedMeasureDetails(
            get().score,
            get().selection,
