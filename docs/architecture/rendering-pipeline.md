...
- Measure traversal, wrapping, glyph planning, curves, hitboxes, and SVG string
  serialization live mostly in `Renderer`.
...
3. Move private width helpers from `Renderer` into layout helpers.
4. Add `SpacingSolver` for column positions and wrapping.
